...
- [x] **Step 1: Прогнать тесты** — `python -m pytest -q` → Expected: `189 passed`
- [x] **Step 2: Коммит**
...
- [x] **Step 1: Написать падающий тест** (в конец tests/test_config.py)
...
- [x] **Step 2: Убедиться, что падает** — `python -m pytest tests/test_config.py -q` → FAIL (нет атрибутов)
- [x] **Step 3: Минимальная реализация** в `config.py`:
...
- [x] **Step 4: Тесты зелёные** — `python -m pytest -q` → PASS (191)
- [x] **Step 5: Обновить examples/config.example.yaml** (после блока `telegram:`):
...
- [x] **Step 1: Написать падающие тесты** (`tests/test_auto.py`)
...
- [x] **Step 2: Убедиться, что падают** — `python -m pytest tests/test_auto.py -q` → FAIL (модуля нет)
- [x] **Step 3: Реализация** `src/content_factory/orchestrator/auto.py`:
...
- [x] **Step 4: Тесты зелёные** — `python -m pytest -q` → PASS
- [x] **Step 5: Коммит** — `git add src/content_factory/orchestrator/auto.py tests/test_auto.py && git commit -m "feat(orchestrator): materialize_auto_tasks — постоянные авто-задачи из конфига"`
...
- [x] **Step 1: Падающий тест** (в tests/test_wiring.py; `_http` научить копить тела запросов)
...
- [x] **Step 2: Убедиться, что падает** — `python -m pytest tests/test_wiring.py -q` → FAIL (нет параметра review_chat)
- [x] **Step 3: Реализация** в `scheduler_run.py`:
...
- [x] **Step 4: Все тесты зелёные** — `python -m pytest -q` → PASS (старые wiring-тесты без review_chat работают: фолбэк owner_chat)
- [x] **Step 5: Коммит** — `git commit -m "feat(scheduler): авто-задачи на каждом тике + превью confirm в ревью-канал"`
...
- [x] **Step 1: Падающий тест** (tests/test_bot_run.py)
...
- [x] **Step 2: Убедиться, что падает** — `python -m pytest tests/test_bot_run.py -q` → FAIL (нет finalize_preview)
- [x] **Step 3: Реализация** в `bot/run.py` (import json вверху):
...
- [x] **Step 4: Все тесты зелёные** — `python -m pytest -q` → PASS
- [x] **Step 5: Коммит** — `git commit -m "feat(bot): вердикт вместо кнопок после ✅/❌ (журнал в ревью-канале)"`
...
- [x] **Step 1: Дописать DEPLOY.md, коммит** — `git commit -m "docs(deploy): включение полного автомата"`
