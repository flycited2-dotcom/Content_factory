...
- Per-lane кнопки start/stop в vps_bot — отложено до реального акк2 (машинного
  уровня + enabled-флага в lanes.json пока достаточно).
- Ноут: перезапустить вотчдог (новый lane-код); Десктоп: `git checkout main && git pull`.
