...
# Группы синонимов: один факт часто назван по-разному в ТТХ и в УТП поставщика.
# Плоский список не ловил такие пары — в живом посте XIGMA SKY Inverter рядом
# вышли «Работа на обогрев до −15 °C» и «✓ Работа на нагрев до -15 С».
_CONCEPTS = (
    ("wi-fi", "wifi", "вай"),
    ("инвертор",),
    ("ионизац",),
    ...
    ("самоочист", "автоочист"),
    ("sleep", "ночн"),
    ("авторестарт", "перезапуск"),
    ("энергоэфф", "класс энерг"),
    ("гаранти",),
    ("компрессор",),
    ("шум",),
    ("голос",),
...
def _dup_of_produced(item_low, produced_low):
    """Пункт УТП уже показан структурным буллетом — пусть и другими словами."""
    for group in _CONCEPTS:
        if (any(c in item_low for c in group)
                and any(c in produced_low for c in group)):
            return True
    return False
