...
STEPS = ("awaiting_qty", "awaiting_qty_custom", "awaiting_comment")
...
class OrderDialogStore:
    ...
    def start(self, chat_id: str, key: str) -> None:
        ...
        with self._c() as c:
            c.execute("INSERT INTO order_dialog(chat_id, step, key, qty) VALUES(?,?,?,NULL) "
                      "ON CONFLICT(chat_id) DO UPDATE SET step=excluded.step, "
                      "key=excluded.key, qty=NULL",
    ...
    def snapshot(self, chat_id: str) -> OrderState | None:
        with self._c() as c:
            row = c.execute("SELECT step, key, qty FROM order_dialog WHERE chat_id=?",
